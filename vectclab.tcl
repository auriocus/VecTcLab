...
# add the current dir as an auto_path
set vectclabdir [file normalize [file dirname [info script]]]
lappend auto_path [file join $vectclabdir lib]
...
proc ::tkcon::InitSlave {slave {slaveargs {}} {slaveargv0 {}}} {
    ...
    global vectclabdir
    set slautopath [list {*}[lremove $auto_path $tk_library] $vectclabdir]
    puts "Slave's autopath: [join $slautopath \n]"
    interp eval $slave set tcl_interactive $tcl_interactive \; \
	    set auto_path [list $slautopath] \; \
	    set argc [llength $slaveargs] \; \
	...
	package require vectcl::plot
	namespace import vectcl::*
	namespace import vectcl::plot::*
	...
	proc ::vectcl::mformat m {
		...
		if {[string length $strrep] > $::DISPLAY_LIMIT} { return "(Too much data)" }
		return $strrep
    }
    set DISPLAY_LIMIT 1000
    }
}
...
proc ::tkcon::EvalCmd {w cmd} {
	...
	if {$OPT(calcmode)} { set exprerr $err }
